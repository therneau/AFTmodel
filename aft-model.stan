...
// This file is extracted (tangled) from 'src/stan-runs.org'
// and modifications to this file will be overwritten
...
data { 
  ...
  real tau;	   // SD of the bivariate t random effects
  real dft;        // degrees of freedom of the bivariate t
  int adrc[N];     // indicator of ADRC (it's int but multiplied by real)
}
parameters {
  ...
  matrix[P,2] beta;
  real rho;
  real adrc_shift;          // how much earlier are ADRC subjects?
}
transformed parameters {
   ...
   lin = x*beta;                                                      // ADRC shift curve same for PiB and tau
   for (i in 1:N) {                                                   // ...................
     tage[i] = (age[i] + alpha[id[i],outcome[i]] + lin[i, outcome[i]] + adrc_shift * adrc[i])/10 - B[4,outcome[i]];
